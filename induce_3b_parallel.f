...
c
c      if (body1) then
c         call induce1_a_3bi (fdir,field,fieldp,dscale,pscale)
c      end if
c      if (body2) then
c         call induce2_a_3bi (fdir,field,fieldp,dscale,pscale)
c      end if
c      if (body3) then
c         call induce3_a_3bi (fdir,field,fieldp)
c      end if
...
c
c Can only time using dtime when compiled with gfortran
c
      timediff = dtime(timearray)
      if(body1) call induce1_3bi (fdir,field,fieldp)
      if(body2) call induce2_3bi (fdir,field,fieldp)
      if(body3) call induce3_3bi (fdir,field,fieldp)
      timediff = dtime(timearray)
      delta1 = delta1 + timediff
...
c
             do while (.not. done)
                ...
                if (pme) then
                   call umutual1_3b_pme (field,fieldp)
                  else
                   call umutual1_3b_reg (field,fieldp)
                end if
                call umutual2a_3b (field,fieldp)
                term = (4.0d0/3.0d0) * aewald**3 / sqrtpi
                do l1 = 1, npole3b
                   i = pnum(l1)
                   do j = 1, 3
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine udirect1_3b_reg  --  Ewald recip direct induced ##
c     ##                                                             ##
c     #################################################################
c
c
c     "udirect1_3b_reg" computes the reciprocal space contribution of
c     the
...
c
c
      subroutine udirect1_3b_reg (field)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine udirect1_3b_pme  --  Ewald recip direct induced field  ##
c     ##                                                             ##
c     #################################################################
c
c
...
c
c
      subroutine udirect1_3b_pme (field)
      implicit none
      include 'sizes.i'
      include 'bound.i'
      include 'boxes.i'
...
c
c
      subroutine udirect2a_3b3 (field,fieldp)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
...
c     #################################################################
c     ##                                                             ##
c     ## subroutine umutual1_3b_reg -- Ewald recip mutual induced field ##
c     ##                                                             ##
c     #################################################################
c
c
c     "umutual1_3b_reg" computes the reciprocal space contribution of the
...
c
c
      subroutine umutual1_3b_reg (field,fieldp)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine umutual1_3b_pme  --  Ewald recip mutual induced field  ##     
c      ##                                                             ##
c     #################################################################
c
c
c     "umutual1_3b_pme" computes the reciprocal space contribution of the
c     induced atomic dipole moments to the field
c
c
      subroutine umutual1_3b_pme (field,fieldp)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
...
c
      call fftback
      call fphi_uind_3b (fdip_phi1,fdip_phi2,fdip_sum_phi)
c
