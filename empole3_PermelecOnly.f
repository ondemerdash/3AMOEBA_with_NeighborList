...
c            call empole3d
c         else
c            call empole3c_PermelecOnly(field,fieldp)
c         end if
c      else
c         if (use_mlist) then
c            call empole3b
...
c
c
      subroutine empole3a_PermelecOnly
      implicit none
      include 'sizes.i'
      include 'action.i'
      include 'analyz.i'
...
c
c
      subroutine empole3b_PermelecOnly
      implicit none
      include 'sizes.i'
      include 'action.i'
      include 'analyz.i'
...
c
c
      subroutine empole3c_PermelecOnly(field,fieldp)
      implicit none
      include 'sizes.i'
      include 'action.i'
      include 'analyz.i'
...
c
      call erecip_PermelecOnly
c
c     compute the real space part of the Ewald summation
c
      call ereal3c_PermelecOnly (eintra)
c
...
c
c
      subroutine ereal3c_PermelecOnly (eintra)
      implicit none
      include 'sizes.i'
      include 'action.i'
      include 'analyz.i'
...
c     calculate the scalar products for polarization components
c
c               sci(2) = uix*dkx + dix*ukx + uiy*dky
c     &                     + diy*uky + uiz*dkz + diz*ukz
c               sci(3) = uix*xr + uiy*yr + uiz*zr
c               sci(4) = ukx*xr + uky*yr + ukz*zr
c               sci(7) = qix*ukx + qiy*uky + qiz*ukz
c               sci(8) = qkx*uix + qky*uiy + qkz*uiz
c
c     calculate the gl functions for permanent multipoles
c
               gl(0) = ci*ck
...
c     calculate the gl functions for polarization components
c
c               gli(1) = ck*sci(3) - ci*sci(4) + sci(2)
c               gli(2) = 2.0d0*(sci(7)-sci(8)) - sci(3)*sc(4)
c     &                     - sc(3)*sci(4)
...
c
     ...
     &                + gl(3)*bn(3) + gl(4)*bn(4)
c               ei = gli(1)*bn(1) + gli(2)*bn(2) + gli(3)*bn(3)
c
...
c
     ...
     &                                    +0.6d0*damp**2)*expdamp)
               ...
               total_field(1,i) = total_field(1,i)
     &                              + ck*xr*rr3*(1.0d0-expdamp)
               total_field(2,i) = total_field(2,i)
     &                              + ck*yr*rr3*(1.0d0-expdamp)
               total_field(3,i) = total_field(3,i)
     &                              + ck*zr*rr3*(1.0d0-expdamp)

               efix = gl(0)*rr1 + gl(1)*rr3 + gl(2)*rr5
     &                   + gl(3)*rr7 + gl(4)*rr9
c               eifix = gli(1)*rr3*(1.0d0-scale3)
c     &                    + gli(2)*rr5*(1.0d0-scale5)
c     &                    + gli(3)*rr7*(1.0d0-scale7)
c
c     apply the energy adjustments for scaled interactions
c
...
c               ep = ep + ei
               aem(ii) = aem(ii) + 0.5d0*e
               aem(kk) = aem(kk) + 0.5d0*e
c               aep(ii) = aep(ii) + 0.5d0*ei
c               aep(kk) = aep(kk) + 0.5d0*ei
c
c     increment the total intramolecular energy
c
               efix = f * efix * mscale(kk)
c               eifix = gli(1)*rr3*scale3 + gli(2)*rr5*scale5
c     &                    + gli(3)*rr7*scale7
c               eifix = 0.5d0 * f * eifix
               if (molcule(ii) .eq. molcule(kk)) then
c                  eintra = eintra + efix + eifix
                  eintra = eintra + efix
               end if
c
c     print a message if the energy of this interaction is large
...
c
c
      subroutine erecip_PermelecOnly
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
...
c                     uk = h1*um(1,i) + h2*um(2,i) + h3*um(3,i)
...
c
c
      subroutine empole3d_PermelecOnly
      implicit none
      include 'sizes.i'
      include 'action.i'
      include 'analyz.i'
