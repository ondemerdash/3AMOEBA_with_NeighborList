...
c
c
      subroutine erecip1_PermelecOnly
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
            ...
            do l = lmin, lmax
               ...
               if (hsq .le. cut) then
                  ...
                  do i = 1, npole
                     ...
                     qky(i) = h1*q3 - h3*q1
                     qkz(i) = h2*q1 - h1*q2
c                     uk = h1*uind(1,i) + h2*uind(2,i) + h3*uind(3,i)
c                     ukp = h1*uinp(1,i) + h2*uinp(2,i) + h3*uinp(3,i)
...
c
                     ...
                     dt(2,3) = h3 * dm(2,i)
                     dt(3,3) = h3 * dm(3,i)
c                     dtu(1,1) = h1 * uind(1,i)
c                     dtu(2,1) = h1 * uind(2,i)
c                     dtu(3,1) = h1 * uind(3,i)
c                     dtu(1,2) = h2 * uind(1,i)
c                     dtu(2,2) = h2 * uind(2,i)
c                     dtu(3,2) = h2 * uind(3,i)
c                     dtu(1,3) = h3 * uind(1,i)
c                     dtu(2,3) = h3 * uind(2,i)
c                     dtu(3,3) = h3 * uind(3,i)
c                     dtp(1,1) = h1 * uinp(1,i)
c                     dtp(2,1) = h1 * uinp(2,i)
c                     dtp(3,1) = h1 * uinp(3,i)
c                     dtp(1,2) = h2 * uinp(1,i)
c                     dtp(2,2) = h2 * uinp(2,i)
c                     dtp(3,2) = h2 * uinp(3,i)
c                     dtp(1,3) = h3 * uinp(1,i)
c                     dtp(2,3) = h3 * uinp(2,i)
c                     dtp(3,3) = h3 * uinp(3,i)
                     do m2 = 1, 3
                        do m1 = 1, 3
                           t5(m1,m2) = t5(m1,m2) - dt(m1,m2)*ckr(i)
     &                                    + 2.0d0*qt(m1,m2)*skr(i)
c                           t5u(m1,m2) = t5u(m1,m2) - dtu(m1,m2)*ckr(i)
c                           t5p(m1,m2) = t5p(m1,m2) - dtp(m1,m2)*ckr(i)
                           t6(m1,m2) = t6(m1,m2) + dt(m1,m2)*skr(i)
     &                                    + 2.0d0*qt(m1,m2)*ckr(i)
c                           t6u(m1,m2) = t6u(m1,m2) + dtu(m1,m2)*skr(i)
c                           t6p(m1,m2) = t6p(m1,m2) + dtp(m1,m2)*skr(i)
                        end do
                     end do
                  end do
c
...
c                     uterm = uterm - expterm*(t3*t3p+t4*t4p)
c                     do m2 = 1, 3
c                        do m1 = 1, 3
c                           wterm(m1,m2) = wterm(m1,m2)
c     &                        - expterm*(t3*t5p(m1,m2)+t3p*t5u(m1,m2)
c     &                                  +t4*t6p(m1,m2)+t4p*t6u(m1,m2))
c                        end do
c                     end do
c                  end if
...
c                     dei = 0.5d0 * expterm * ((s4(i)+s4p(i))*t1
c     &                                       -(s3(i)+s3p(i))*t2
c     &                                +s2(i)*(t3+t3p)-s1(i)*(t4+t4p))
c                     if (poltyp .eq. 'MUTUAL') then
c                         dei = dei + 0.5d0 * expterm
c     &                            * (s4p(i)*t3+s4(i)*t3p
c     &                              -s3p(i)*t4-s3(i)*t4p)
c                     end if
                     det1 = expterm * (skr(i)*t2-ckr(i)*t1)
                     det2 = 2.0d0 * expterm * (ckr(i)*t2+skr(i)*t1)
c                     det1i = 0.5d0 * expterm * (skr(i)*(t4+t4p)
c     &                                         -ckr(i)*(t3+t3p))
c                     det2i = expterm * (ckr(i)*(t4+t4p)+skr(i)*(t3+t3p))
                     dem(1,ii) = dem(1,ii) + h1*de
                     dem(2,ii) = dem(2,ii) + h2*de
                     dem(3,ii) = dem(3,ii) + h3*de
c                     dep(1,ii) = dep(1,ii) + h1*dei
c                     dep(2,ii) = dep(2,ii) + h2*dei
c                     dep(3,ii) = dep(3,ii) + h3*dei
                     trq(1,ii) = trq(1,ii) + dkx(i)*det1 + qkx(i)*det2
                     trq(2,ii) = trq(2,ii) + dky(i)*det1 + qky(i)*det2
                     trq(3,ii) = trq(3,ii) + dkz(i)*det1 + qkz(i)*det2
c                     trqi(1,ii) = trqi(1,ii) + dkx(i)*det1i
c     &                               + qkx(i)*det2i
c                     trqi(2,ii) = trqi(2,ii) + dky(i)*det1i
c     &                               + qky(i)*det2i
c                     trqi(3,ii) = trqi(3,ii) + dkz(i)*det1i
c     &                               + qkz(i)*det2i
                  end do
               end if
            end do
