...
c
      ...
      integer k1,k2
      integer moli1
c      real*8 ep3bt,dep3bt(3,npole),virep3bt(3,3)
c      real*8 ep3b,dep3b(3,npole),virep3b(3,3)    
...
c      open(unit=epvr1,file='epvr_2body.txt')
c      open(unit=epvr2,file='epvr_3body.txt')
c
c
...
c    COMMENTED OUT WHILE LOOP
c      do while (.not. abort)
c         frame = frame + 1
c         if (frame .gt. 1) then
c            write (iout,90)  frame
c   90       format (/,' Analysis for Archive Structure :',8x,i8)
c         end if
...
c        if(domollst2bod) call molfull2body

c        if(domollst3bod) call molfull3bodycobar 

       tottriples=0
       if (i3.eq.1) then
         do k=1,nmol
          ntriples=nmollst3mod(k)/2
c          print*,"Num triples moli1",k,ntriples
          print*,"Nmollst3mod",k,nmollst3mod(k)
          tottriples=tottriples+ntriples
         end do
         print*,"Triplecount=",tottriples
       end if
...
c      do i = 1, n
c          print*,'dep in analysis x', i, dep(1,i)
c          print*,'dep in analysis y', i, dep(2,i)
c          print*,'dep in analysis z', i, dep(3,i)
c      end do

c      print*, 'Virialxx=',vir(1,1)
c      print*, 'Virialxy=',vir(1,2)
c      print*, 'Virialxz=',vir(1,3)
c      print*, 'Virialyx=',vir(2,1)
c      print*, 'Virialyy=',vir(2,2)
c      print*, 'Virialyz=',vir(2,3)
c      print*, 'Virialzx=',vir(3,1)
c      print*, 'Virialzy=',vir(3,2)
c      print*, 'Virialzz=',vir(3,3)

c      write(6,*)'Total PE',ec,ecd,ed,em,ep
c      print*,"Total Potential Energy :",energy,"Kcal/mole"
      ethg(i3)=esum
      fthg(i3)=desum(1,3)
      epArr(i3)=ep
...
c COMMENT OUT PARTYZE,PROPYZE, READXYZ, AND WHILE END DO 
c
