...
c
c
      subroutine empole1c_3b_PolelecOnly(field,fieldp)
      implicit none
      include 'sizes.i'
      include 'atoms.i'
      include 'boxes.i'
...
c
      call induce0c_3b_PolelecOnly(field,fieldp)
c
...
c      do l1 = 1, npole3b
c         i = pnum(l1)
c         write(6,*)'dep',dep(1,i),dep(2,i),dep(3,i)
c      end do
c
...
c
      term = 2.0d0 * aewaldPerm * aewaldPerm
      fterm = -f * aewaldPerm / sqrtpi
      do l1 = 1, npole3b
         i = pnum(l1)
         ci = rpole(1,i)
         dix = rpole(2,i)
...
c
      ...
      trq(2) = 0.0d0
      trq(3) = 0.0d0
      term = (4.0d0/3.0d0) * f * aewaldPerm**3 / sqrtpi
      do l1 = 1, npole3b
         i = pnum(l1)
         dix = rpole(2,i)
         diy = rpole(3,i)
...
c      print*,moli1,moli2,moli3
c      print*,"hello",ep
c       do l1 = 1, npole3b
c          i = pnum(l1)
c          print*,"dep",dep(1,i),dep(2,i),dep(3,i)
c       end do
c
...
c
               ralpha = aewaldPerm * r
               bn(0) = erfc(ralpha) / r
               alsq2 = 2.0d0 * aewaldPerm**2
               alsq2n = 0.0d0
               if (aewaldPerm .gt. 0.0d0) then
                 alsq2n = 1.0d0 / (sqrtpi*aewaldPerm)
               end if
               exp2a = exp(-ralpha**2)
               do j = 1, 5
                  bfac = dble(2*j-1)
...
c     &                      + gfr(4)*(qkdi(1)-qidk(1)) + gfr(5)*qir(1)
c     &                      + gfr(6)*qkr(1) + gfr(7)*(qiqkr(1)+qkqir(1))
c               ftm2r(2) = gfr(1)*yr + gfr(2)*di(2) + gfr(3)*dk(2)
c     &                      + gfr(4)*(qkdi(2)-qidk(2)) + gfr(5)*qir(2)
c     &                      + gfr(6)*qkr(2) + gfr(7)*(qiqkr(2)+qkqir(2))
c               ftm2r(3) = gfr(1)*zr + gfr(2)*di(3) + gfr(3)*dk(3)
c     &                      + gfr(4)*(qkdi(3)-qidk(3)) + gfr(5)*qir(3)
c     &                      + gfr(6)*qkr(3) + gfr(7)*(qiqkr(3)+qkqir(3))
c
...
c     &           - gf(5)*rxqir(1) - gf(7)*(rxqikr(1)+qkrxqir(1))
c               ttm2(2) = -bn(1)*dixdk(2) + gf(2)*dixr(2)
c     &           + gf(4)*(dixqkr(2)+dkxqir(2)+rxqidk(2)-2.0d0*qixqk(2))
c     &           - gf(5)*rxqir(2) - gf(7)*(rxqikr(2)+qkrxqir(2))
c               ttm2(3) = -bn(1)*dixdk(3) + gf(2)*dixr(3)
c     &           + gf(4)*(dixqkr(3)+dkxqir(3)+rxqidk(3)-2.0d0*qixqk(3))
c     &           - gf(5)*rxqir(3) - gf(7)*(rxqikr(3)+qkrxqir(3))
c               ttm3(1) = bn(1)*dixdk(1) + gf(3)*dkxr(1)
c     &           - gf(4)*(dixqkr(1)+dkxqir(1)+rxqkdi(1)-2.0d0*qixqk(1))
c     &           - gf(6)*rxqkr(1) - gf(7)*(rxqkir(1)-qkrxqir(1))
c               ttm3(2) = bn(1)*dixdk(2) + gf(3)*dkxr(2)
c     &           - gf(4)*(dixqkr(2)+dkxqir(2)+rxqkdi(2)-2.0d0*qixqk(2))
c     &           - gf(6)*rxqkr(2) - gf(7)*(rxqkir(2)-qkrxqir(2))
c               ttm3(3) = bn(1)*dixdk(3) + gf(3)*dkxr(3)
c     &           - gf(4)*(dixqkr(3)+dkxqir(3)+rxqkdi(3)-2.0d0*qixqk(3))
c     &           - gf(6)*rxqkr(3) - gf(7)*(rxqkir(3)-qkrxqir(3))
c
c     get the permanent torque without screening
c
c               ttm2r(1) = -rr3*dixdk(1) + gfr(2)*dixr(1)-gfr(5)*rxqir(1)
c     &           + gfr(4)*(dixqkr(1)+dkxqir(1)+rxqidk(1)-2.0d0*qixqk(1))
c     &           - gfr(7)*(rxqikr(1)+qkrxqir(1))
c               ttm2r(2) = -rr3*dixdk(2) + gfr(2)*dixr(2)-gfr(5)*rxqir(2)
c     &           + gfr(4)*(dixqkr(2)+dkxqir(2)+rxqidk(2)-2.0d0*qixqk(2))
c     &           - gfr(7)*(rxqikr(2)+qkrxqir(2))
c               ttm2r(3) = -rr3*dixdk(3) + gfr(2)*dixr(3)-gfr(5)*rxqir(3)
c     &           + gfr(4)*(dixqkr(3)+dkxqir(3)+rxqidk(3)-2.0d0*qixqk(3))
c     &           - gfr(7)*(rxqikr(3)+qkrxqir(3))
c               ttm3r(1) = rr3*dixdk(1) + gfr(3)*dkxr(1) -gfr(6)*rxqkr(1)
c     &           - gfr(4)*(dixqkr(1)+dkxqir(1)+rxqkdi(1)-2.0d0*qixqk(1))
c     &           - gfr(7)*(rxqkir(1)-qkrxqir(1))
c               ttm3r(2) = rr3*dixdk(2) + gfr(3)*dkxr(2) -gfr(6)*rxqkr(2)
c     &           - gfr(4)*(dixqkr(2)+dkxqir(2)+rxqkdi(2)-2.0d0*qixqk(2))
c     &           - gfr(7)*(rxqkir(2)-qkrxqir(2))
c               ttm3r(3) = rr3*dixdk(3) + gfr(3)*dkxr(3) -gfr(6)*rxqkr(3)
c     &           - gfr(4)*(dixqkr(3)+dkxqir(3)+rxqkdi(3)-2.0d0*qixqk(3))
...
c
               do j = 1, 3
c                  ftm2(j) = f * (ftm2(j)-(1.0d0-mscale(kk))*ftm2r(j))
                  ftm2i(j) = f * (ftm2i(j)-ftm2ri(j))
c                  ttm2(j) = f * (ttm2(j)-(1.0d0-mscale(kk))*ttm2r(j))
                  ttm2i(j) = f * (ttm2i(j)-ttm2ri(j))
c                  ttm3(j) = f * (ttm3(j)-(1.0d0-mscale(kk))*ttm3r(j))
...
c      em = em + e
      do l1 = 1, npole3b
         i = pnum(l1)
         ii = ipole(i)
c         dem(1,ii) = dem(1,ii) + frc(1,i)
c         dem(2,ii) = dem(2,ii) + frc(2,i)
c         dem(3,ii) = dem(3,ii) + frc(3,i)
      end do
c
...
c
     ...
     &                 - cmp(8,i)*cphi(6,i) - cmp(9,i)*cphi(10,i)
      ...
      call torque2_3b (trq,frc)
      do l1 = 1, npole3b
         i = pnum(l1)
c         dem(1,i) = dem(1,i) + frc(1,i)
c         dem(2,i) = dem(2,i) + frc(2,i)
c         dem(3,i) = dem(3,i) + frc(3,i)
      end do
c
...
c
         call fftback
         call fphi_uind_3b (fphid,fphip,fphidp)
         do l1 = 1, npole3b
            i = pnum(l1)
            do j = 1, 10
...
c
         ...
         call fphi_to_cphi_3b (fphidp,cphi)
c
...
c                     dem(1,ii) = dem(1,ii) + h1*de
c                     dem(2,ii) = dem(2,ii) + h2*de
c                     dem(3,ii) = dem(3,ii) + h3*de
