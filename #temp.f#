...
c
c     compute the induced dipole moment at each atom for 3-body only
c
...
c
               ...
               if (ii .eq. kk) then
               do j = 1, 3
                  ftm2i(j) = 0.5d0 * (ftm2i(j)-ftm2ri(j))
                  ttm2i(j) = 0.5d0 * (ttm2i(j)-ttm2ri(j))
                  ttm3i(j) = 0.5d0 * (ttm3i(j)-ttm3ri(j))
               end do
               end if    
c
c     increment gradient due to force and torque on first site
